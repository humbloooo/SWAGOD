...
export default function ProductClient({ id }: ProductClientProps) {
    ...
    if (loading) return (
        <main className="min-h-screen bg-background pt-32 pb-[100px] overflow-hidden">
            <Header />
            <div className="container mx-auto px-6">
                <div className="mb-12">
                    <div className="h-4 bg-foreground/5 w-64 animate-pulse rounded"></div>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-32">
                    <div className="space-y-6">
                        <div className="relative aspect-[3/4] bg-foreground/5 animate-pulse border border-foreground/10"></div>
                        <div className="grid grid-cols-4 gap-4">
                            {[...Array(4)].map((_, i) => (
                                <div key={i} className="relative aspect-square bg-foreground/5 animate-pulse border border-foreground/10"></div>
                            ))}
                        </div>
                    </div>
                    <div className="flex flex-col space-y-12">
                        <div className="space-y-6">
                            <div className="h-20 bg-foreground/5 animate-pulse w-3/4"></div>
                            <div className="h-10 bg-foreground/5 animate-pulse w-1/3"></div>
                        </div>
                        <div className="space-y-4">
                            <div className="h-12 bg-foreground/5 animate-pulse"></div>
                            <div className="h-12 bg-foreground/5 animate-pulse"></div>
                        </div>
                        <div className="h-16 bg-foreground/5 animate-pulse w-full"></div>
                    </div>
                </div>
            </div>
            <Navigation />
    ...
    if (!product) return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center text-foreground p-6">
            ...
            <p className="font-mono text-xs text-foreground/40 mb-8 uppercase tracking-widest">The requested item is currently unavailable.</p>
            <Link href="/shop" className="px-8 py-4 bg-primary text-background font-black uppercase tracking-widest hover:bg-foreground transition-all">
                BROWSE PRODUCTS
            </Link>
        </div>
    );
    ...
    return (
        <main className="min-h-screen bg-background text-foreground pb-[100px] pt-32">
            <ProductSchema product={product} />
            <Header />
            <div className="container mx-auto px-6">
                <div className="mb-12">
                    <nav className="flex items-center gap-2 text-[10px] font-mono uppercase text-foreground/40">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-32">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                {product.images.map((img, idx) => (
                                    <button
                                        ...
                                        className={`relative aspect-square border transition-all ${selectedImage === img ? "border-primary" : "border-foreground/10 hover:border-foreground/30"}`}
                                    >
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="mb-12 space-y-2 border-l border-foreground/10 pl-8">
                            <Accordion title="PRODUCT DETAILS">
                                <div className="grid grid-cols-2 gap-y-4 font-mono text-[10px] uppercase tracking-widest text-foreground/60 pt-4">
                                    <div>CATEGORY:</div><div className="text-foreground">{product.category}</div>
                                    <div>REGION:</div><div className="text-foreground">SOUTH AFRICA</div>
                                    <div>STATUS:</div><div className="text-foreground text-primary">AUTHENTIC</div>
                                </div>
                            </Accordion>

                            <Accordion title="DESCRIPTION">
                                <p className="text-foreground/60 font-mono text-sm leading-relaxed uppercase max-w-lg pt-4 pb-2">
                                    {product.description}
                                </p>
                            </Accordion>
                        </div>
                        ...
                        {product.sizes && product.sizes.length > 0 && (
                            <div className="mb-12">
                                <h3 className="text-[10px] font-mono text-foreground/20 uppercase tracking-[0.3em] mb-6">SELECT SIZE:</h3>
                                    ...
                                    {product.sizes.map((size) => (
                                        <button
                                            ...
                                            className={`w-14 h-14 flex items-center justify-center border font-black transition-all ${selectedSize === size
                                                ? "bg-primary text-background border-primary scale-110 shadow-[0_0_20px_rgba(255,100,0,0.3)]"
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="space-y-4">
                                <button
                                    ...
                                    className="text-[10px] font-mono text-foreground/20 uppercase tracking-widest hover:text-primary transition-colors text-left flex items-center gap-2"
                                >
                        ...
                        <div className="mt-16 grid grid-cols-2 gap-8 border-t border-foreground/5 pt-12">
                            <div className="space-y-2">
                                <p className="text-[8px] font-mono text-foreground/20 uppercase tracking-widest">SHIPPING_PRIORITY</p>
                                ...
                                <p className="text-[10px] font-mono text-foreground/60 uppercase tracking-widest">GOVERNMENT COMPLIANT {"//"} FREE OVER R2000</p>
                            </div>
                            <div className="space-y-2">
                                <p className="text-[8px] font-mono text-foreground/20 uppercase tracking-widest">RETURN_POLICY</p>
                                ...
                                {relatedProducts.map((rp) => (
                                    <Link href={`/product/${rp.id!}`} key={rp.id} className="group block relative overflow-hidden bg-foreground/5 border border-foreground/10 p-6">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="font-mono text-xs text-foreground/40">{formatPrice(rp.price, currency)}</p>
                                        </div>
                                    </Link>
                                ))}
                            </div>
                ...
                <AnimatePresence>
                    {isLightboxOpen && (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-xl flex flex-col p-6 items-center justify-center cursor-zoom-out"
                        ...
                        >
                            ...
                            <motion.div
                                ...
                                className="relative w-full max-w-4xl max-h-[85vh] aspect-[3/4] md:aspect-auto md:h-full border border-foreground/10"
                ...
                <AnimatePresence>
                    {showStickyCart && (
                        <motion.div
                            ...
                            className="fixed bottom-[60px] left-0 right-0 z-40 bg-background/90 backdrop-blur-xl border-t border-foreground/10 p-4 md:hidden"
                        >
...
function Accordion({ title, children }: { title: string, children: React.ReactNode }) {
    ...
    return (
        <div className="border-b border-foreground/10 last:border-0 py-2">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="w-full flex items-center justify-between text-left py-2 hover:text-primary transition-colors group"
            >
                <h3 className="text-[10px] font-mono text-foreground/40 group-hover:text-primary transition-colors uppercase tracking-[0.3em]">{title}</h3>
