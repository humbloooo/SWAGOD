            ...
            <div className="container mx-auto max-w-5xl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
                    ...
                    <button
                        ...
                        className="px-10 py-5 bg-primary text-background font-black uppercase tracking-widest hover:bg-foreground transition-all shadow-xl glitch-hover"
                    >
                        + ADD NEW EVENT
                    </button>
                </div>
                ...
                {isEditing && (
                    <div className="fixed inset-0 bg-background/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
                        <div className="bg-background border border-foreground/20 p-10 max-w-xl w-full relative shadow-2xl">
                            <h2 className="text-4xl font-black uppercase tracking-tighter mb-10 border-b border-foreground/10 pb-4">INITIALIZE // EVENT</h2>
                            <form onSubmit={handleSubmit} className="space-y-8 font-mono text-xs uppercase tracking-widest">
                                <div className="space-y-2">
                                    <label className="text-foreground/40">TEMPORAL_MARKER (DATE)</label>
                                    <input
                                        type="date"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="text-foreground/40">CITY_NODE</label>
                                        <input
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-foreground/40">VENUE_STATION</label>
                                        <input
                                            className="w-full bg-foreground/5 border border-foreground/10 p-4 focus:border-primary outline-none transition-all"
                                            value={newItem.venue || ""}
                                ...
                                <div className="space-y-2">
                                    <label className="text-foreground/40">PURCHASE_LINK_URI (OPTIONAL)</label>
                                    <input
                                        className="w-full bg-foreground/5 border border-foreground/10 p-4 focus:border-primary outline-none transition-all"
                                ...
                                <div className="flex justify-end gap-6 pt-10">
                                    <button
                                        ...
                                        className="px-10 py-5 border border-foreground/20 font-black tracking-widest hover:bg-foreground/5 transition-all text-[10px]"
                                    >
                                        ABORT
                                    </button>
                                    <button
                                        type="submit"
                                        className="px-10 py-5 bg-primary text-background font-black tracking-widest hover:bg-foreground transition-all text-[10px]"
                                    >
                                        EXECUTE_WRITE
                                    </button>
                                </div>
                ...
                <div className="space-y-6">
                    {tours.map((tour, i) => (
                        <div key={i} className="flex items-center justify-between p-8 border border-foreground/10 bg-foreground/5 backdrop-blur-md hover:border-primary transition-all duration-300 group">
                            <div>
                    ...
                    {tours.length === 0 && (
                        <div className="text-center py-24 border border-foreground/10 border-dashed opacity-30 font-mono uppercase tracking-[0.3em] italic">
