...
export default function AdminFeedback() {
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center">
            <div className="text-foreground font-mono animate-pulse tracking-widest text-2xl uppercase">LOADING INTEL...</div>
        </div>
    );
        ...
        <main className="pb-[60px] md:pb-[100px] pt-24 md:pt-32 px-4 md:px-6">
            <div className="container mx-auto max-w-6xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
                    {feedback.length === 0 && (
                        <div className="col-span-full py-24 border border-foreground/10 flex flex-col items-center justify-center space-y-4">
                            <MessageSquare size={40} className="text-foreground/20" />
                            <p className="text-foreground/30 font-mono uppercase italic tracking-widest text-center text-[10px]">NO DATA RECOVERED.</p>
                        </div>
                    )}
                    ...
                    <AnimatePresence mode="popLayout">
                        {feedback.map((item, idx) => (
                            <motion.div
                                ...
                                className="group relative border border-foreground/10 bg-foreground/5 backdrop-blur-md p-4 md:p-8 hover:border-primary transition-all duration-500 overflow-hidden"
                            >
                                ...
                                <div className="space-y-4 md:space-y-6">
                                    <div className="flex flex-wrap gap-2 md:gap-4 text-[8px] md:text-[10px] font-mono uppercase tracking-widest text-foreground/50">
                                        <div className="flex items-center gap-1.5 md:gap-2 border border-foreground/10 px-1.5 md:px-2 py-0.5 md:py-1 bg-foreground/5">
                                    ...
                                    <div className="relative">
                                        <p className="font-mono text-xs md:text-sm leading-relaxed text-foreground/80 whitespace-pre-wrap pl-4 md:pl-6 border-l-2 border-primary/30 group-hover:border-primary transition-colors">
                                            {item.message}
                                        </p>
                                    </div>
                                </div>
