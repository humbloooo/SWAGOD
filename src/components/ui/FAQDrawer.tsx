            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] bg-background/90 backdrop-blur-xl flex justify-end"
                    ...
                    >
                        ...
                        >
                            <div className="flex justify-between items-center mb-16">
                                ...
                                <button onClick={() => setIsOpen(false)} className="text-foreground/40 hover:text-foreground"><X size={32} /></button>
                            </div>

                            <div className="space-y-4">
                                {FAQS.map((faq, idx) => (
                                    <div key={idx} className="border-b border-foreground/5 py-4">
                                        <button
                                        ...
                                        >
                                            ...
                                            <ChevronDown size={16} className={`transition-transform duration-300 ${activeIndex === idx ? 'rotate-180 text-primary' : 'text-foreground/20'}`} />
                                        </button>
                                        <AnimatePresence>
                                            {activeIndex === idx && (
                                                <motion.div
                                                ...
                                                >
                                                    <p className="font-mono text-[10px] text-foreground/40 uppercase tracking-widest leading-loose pt-4 pb-2">
                                                        {faq.a}
                                                    </p>
                                                </motion.div>
                                            )}
                            ...
                            <div className="mt-32 pt-16 border-t border-foreground/5">
                                <p className="font-mono text-[10px] text-foreground/20 uppercase tracking-[0.3em] mb-4 text-center">DIRECT CONTACT:</p>
